...
/*   Updated: 2022/11/09 15:31:08 by slaszlo-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void ft_drawn_line (char **map_data, t_map *map);
...
int	main(int argc, char *argv[])
{
	...
	map->mlx = mlx_init(WIDTH, HEIGHT, "MLX42", true);
	map->map_data = ft_get_map(argv);
	ft_drawn_line(map->map_data, map);
	mlx_loop(map->mlx);
	mlx_terminate(map->mlx);
	return (0);
}
...
void ft_drawn_line (char **map_data, t_map *map)
{
	...
	while (m_data[i][j] != '\0')
	{
		ft_printf("%c", m_data[i][j]);
		if (m_data[i][j] == '1')
		{
			mlx_image_to_window(map->mlx, blocks, (j * 50), 0);
		}
		j++;
	}
}